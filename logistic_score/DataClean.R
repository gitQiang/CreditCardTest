...
DataClean_Train <- function(dt, flag){
        ...
        if(flag=="yidong"){
                ...
                newindexes_yidong <- c("性别", "借记卡余额总量", "借记卡3个月出账金额", "借记卡3个月入账金额", "借记卡12个月出账金额", "借记卡12个月入账金额", "信用卡数量", "借贷比", "借记卡入账长短比", "借记卡出账长短比", "借记卡3个月出入比", "借记卡12个月出入比", "label")
        ...
        if(!(flag %in% c("yidong","liantong"))){
                newindex_id <- c("性别", "移动", "联通", "华北区", "东北区",   "华东区", "华中华南", "西南区", "label")
                dt1 <- dt[,newindex_id]  
        }
