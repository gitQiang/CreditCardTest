...
getBreaks <- function(){
        ...
        ## 金融画像远期字段得分映射
        jinrongziduan <- read.csv("金融画像衍生字段表_LY_0723_Chang.csv",fileEncoding="utf8")
        ...
        ## 金融画像近期字段得分映射
        jinrongziduan1 <- read.csv("金融画像衍生字段表_LY_0723_Duan.csv",fileEncoding="utf8")
        ...
        #航空字段的得分 
        hangkongziduan <- read.csv("航空数据衍生指标得分划分.csv",strip.white = TRUE,fileEncoding="utf8")
...
credit_scores <- function(fields1,nleaf){
        ...
        colnames(newdata) <- paste("X",1:nleaf[4],sep="")
        result[4] <- predict(fit,as.data.frame(newdata),type="prob")[,2]
...
readxy <- function(samflag=2){
        ...
        yanshengM <- as.matrix(read.csv(paste("衍生指标矩阵_hq",samflag,".csv",sep=""),fileEncoding="utf8"))
        tmp <- as.matrix(read.csv(paste("衍生打分矩阵_hq",samflag,".csv",sep=""),fileEncoding="utf8"))
...
city <- function(s){
        d <- read.csv('city_grade.csv', header = T,fileEncoding="utf8")
...
idcard <- function(s){
        d2<-read.csv('id_city.csv',header = T,fileEncoding="utf8") # 前4位-城市对应表
...
Hangkong_y1_5 <- function(a,b,a0){
        ...
        if(is.na(a) | is.na(b) | a=="" | b==""){
                ...
                ct<-read.csv('id_city.csv',header=T,fileEncoding="utf8")
